...
mixin ArgsMixin {
  final List<String> _args = RefreshedCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(RefreshedCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return RefreshedCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < RefreshedCli.arguments.length) {
      index++;
      return RefreshedCli.arguments[index];
    } else {
